...
import torch
import pdb
#from mapping.mapping_Marques.rendering_utils import render_depth_and_normals,get_camera_rays
from placeability_scoring.mapping.mapping_Marques.rendering_utils import (
    render_depth_and_normals,
    get_camera_rays,
)
...
class Reconstruction:
    ...
    def extract_point_cloud(self,return_raw_logits = False, return_weight=False):
        ...
        weights = None
        if return_weight:
            w_attr, _ = get_properties(self.vbg, target_points, 'weight', res=self.res, voxel_size=self.voxel_size, device=self.device)
            if w_attr is not None:
                weights = w_attr.cpu().numpy().flatten()
        ...
        if return_weight:
            return pcd, ret_labels, weights
        return pcd, ret_labels
